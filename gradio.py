...
def process(
    ...
    with torch.no_grad():
        ...
        style = torch.from_numpy(style_image.copy()).float().cuda() / 127.5 - 1.0
        style = torch.stack([style for _ in range(num_samples)], dim=0)
        style = einops.rearrange(style, "b h w c -> b c h w").clone()

        adm_cond, adm_uc, w = make_conditionings_from_input(style, num=num_samples)
        adm_cond = adm_cond / weights
...
with block:
    ...
    with gr.Row():
        with gr.Column():
            input_image = gr.Image(source="upload", type="numpy", label="structure")
