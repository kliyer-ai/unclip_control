...
class ControlLDM(ImageEmbeddingConditionedLatentDiffusion):
    ...
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, return_keys=None,
        ...
        c_cat, c_crossattn, c_adm = c["c_concat"][0][:N], c["c_crossattn"][0][:N], c["c_adm"][:N]
        N = min(z.shape[0], N)
        n_row = min(z.shape[0], n_row)
        log["reconstruction"] = self.decode_first_stage(z)
        log["control"] = c_cat * 2.0 - 1.0
        log["style"] = batch[self.embed_key]
        log["conditioning"] = log_txt_as_img((512, 512), batch[self.cond_stage_key], size=16)
      
        uc_cross = self.get_unconditional_conditioning(N)
        uc_cat = c_cat  # torch.zeros_like(c_cat)
        uc_full = {"c_concat": [uc_cat], "c_crossattn": [uc_cross], "c_adm": c_adm}
        samples_cfg, _ = self.sample_log(cond={"c_concat": [c_cat], "c_crossattn": [c_crossattn], "c_adm": c_adm},
